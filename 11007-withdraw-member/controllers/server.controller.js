                    ...
                    if (Counter.length <= counter_config) {
                        ...
                        if (withdraw >= min_config) {
                            if (withdraw <= max_config) {
                                ...
                                let checkturnover = await functions.checkturnover(payload.username, withdraw_configs[0], getlastdeposit[0].ref_id).catch(() => { throw err });
                                ...
                                const turn = Double(hdp.turn) + Double(mixParlay.turn) + Double(mixStep.turn) + Double(casino.turn) + Double(slot.turn) + Double(card.turn) + Double(lotto.turn) + Double(keno.turn) + Double(trade.turn) + Double(poker.turn)
                                ...
                                let suspendstatus = await functions.changestatus(payload.username, withdraw_configs[0]).catch(() => { throw err });
