            ...
            .aggregate([
                ...
                },
                {
                    ...
                    $project: {
                        ...
                        username_provider: "$username_provider",
                        profile: "$profile",
                        banking_account: { bank_id: "$banking._id", account_bank_id: "$banking_account.account_bank_id", account_number: "$banking_account.account_number", account_name: "$banking_account.account_name", bank_code: "$banking.code" },
                        financial: "$financial",
                        status: "$status",
                        status_newmember: "$status_newmember",
                        create_date: "$create_date",
...
module.exports.getallmemb = (body) => {
    // console.log(body);
    // let skip = 0;
    // skip = (Number(body.page) - 1) * Number(body.range);
    // console.log(skip, Number(body.range) + skip)
...
module.exports.page_option = (body) => {
    let skip = 0;
    skip = (Number(body.page) - 1) * Number(body.range);
    return new Promise(async (resolve, reject) => {
        await MongoDB.collection(collectionmember)
            .aggregate([
                ...
                $project: {
                    ...
                    now_page: { $toInt: body.page },
                    total_match_page: { $toInt: { $ceil: { $divide: ["$total_match", { $toInt: body.range }] } } },
                    start_rec: { $toInt: (skip + 1) },
                    end_rec: { $toInt: { $cond: [{ $gte: [(Number(body.range) + skip), "$total_match"] }, "$total_match", (Number(body.range) + skip)] } }
                }
            }]).toArray()
            .then(result => resolve(result))
            .catch(error => reject(error));
